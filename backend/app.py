...
async def recent_messages():
    try:
        ...
        return [{"user_message": msg.user_message, "response_message": msg.response_message, "timestamp": msg.timestamp} for msg in messages]
    except Exception as e:
...
async def generate_plan_reasoning(request: InfoRequest):
    try:
        # Retrieve the most recent message from the database
        messages = get_recent_messages(limit=1)
        if not messages:
            raise HTTPException(status_code=404, detail="No recent plan found")
        
        # The most recent message should be the first in the list
        recent_plan = messages[0].response_message
...
async def generate_topic_explanation(request: UserMessageRequest):
    try:
        ...
        if not recent_plan:
            logger.error("No study plan found in stored_plans")
        ...
        logger.error(f"Error in /topic-explanations: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
