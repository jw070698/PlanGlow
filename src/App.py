...
'''@app.post('/checkResourceBlog')
async def generate_check_response_blog(request: CheckRequestBlog):
    check_message_blog = request.check_message_blog
    print("Received request for /CheckRequestBlog")
    try:
        response_check = google_search_availability(check_message_blog)
        return {"response": response_check}
    except Exception as e:
        print(f"Error: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
